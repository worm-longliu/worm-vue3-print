...
<h3>修复</h3>
<ul>
  <li><strong>表格下方间距不再变大</strong>：放在表格下方的元素（合计行、签名栏等），此前打印到纸上时间距会被莫名拉大（设计台上看着正常）。现在以设计台上看到的表格底部为准，出纸与所见一致。</li>
  <li><strong>翻页后不再互相叠压</strong>：表格断页续到新一页后，页尾的组合元素此前会和续排内容叠在一起。现在换页后各元素从上到下顺排，组合依然整组同页、放不下就整组移到下一页。</li>
</ul>

<h3>改进</h3>
<ul>
  <li><strong>组合与多选更好用</strong>：画布和图层面板都支持 Ctrl/⌘ 点击加选；只要选中内容里有组成员，「取消组合」就会出现；组合/取消组合可在工具栏、图层面板底部按钮或 <code>Ctrl+G</code> / <code>Ctrl+Shift+G</code> 完成。帮助文档补充了框选规则与组合说明。</li>
  <li><strong>图标焕新</strong>：工具栏与图层面板图标统一为新风格，全部带悬停提示；组合/取消组合使用专属图标。</li>
</ul>
