...
- `@worm-vue3-print/canvas`: The watermark panel now has two states — fixed text and field expression. Field expressions are edited through the expression dialog (button or double-click on the input), where fields and print date/time variables can be picked directly. The preset field dropdown and the timestamp switch were removed (use `{printDate}`/`{printTime}` in the expression instead); density (dense/medium/loose/custom tile size) and the other settings remain.
